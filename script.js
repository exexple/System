 ...
 * Banner Ads + Interstitial Ads + Task Management + Ranking System
 */
...
let streakDays = 0;
let lastLoginDate = localStorage.getItem('atherionLastLogin') || new Date().toDateString();
...
function shouldShowInterstitial() {
    if (isPremiumActive()) return false; // No ads for premium
...
function renderBannerAds() {
    ...
    // Always show for free, show for ad-premium
    const shouldShowBanner = (premiumType === 'free' || premiumType === 'ad-premium');
    ...
    const bannerHTML = `
        <div style="
            ...
            margin-bottom: 10px;
        ">
            <small style="color: #FBBF24; font-weight: 700;">📢 ADVERTISEMENT</small>
            <p style="color: #A855F7; margin: 6px 0; font-size: 0.85em;">
                ${premiumType === 'free' ? 'Watch 10 ads to unlock 7-day Premium!' : 'Premium Ad Space'}
            </p>
        </div>
    `;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    updateUserDisplay();
    renderBannerAds();
    showInterstitialAd(); // Show on app open
    setupEventListeners();
...
function buyPremium() {
    ...
    // premiumType = 'paid-premium';
    // saveToLocalStorage();
}

function checkMidnightPenalty() {
    const today = new Date().toDateString();
    const lastLogin = localStorage.getItem('atherionLastLogin') || today;
    
    console.log('Today:', today);
    console.log('Last Login:', lastLogin);
    
    if (lastLogin !== today) {
        console.log('🌙 NEW DAY DETECTED - Applying penalty!');
...
function loadFromLocalStorage() {
    try {
        ...
        console.log('✅ ATHERION loaded - Premium:', premiumType);
    } catch (error) {
        console.error('Error:', error);
    }
}
...
window.atherionDebug = {
    ...
    setPremium: (type) => { premiumType = type; updateUserDisplay(); renderBannerAds(); },
