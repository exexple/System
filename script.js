...
function watchAd() {
  ...
  if (isPremium) {
    alert('🎉 You already have Premium! Premium expires in ' + Math.ceil((appData.premiumExpiry - Date.now()) / (1000 * 60 * 60 * 24)) + ' days');
    return;
  }
  
  if (appData.adsWatched >= REWARD_AD_CONFIG.adsRequired) {
    alert('🎉 You have already watched all 10 ads! Premium unlocked for 7 days!');
    return;
  }
  ...
  if (appData.adsWatched >= REWARD_AD_CONFIG.adsRequired) {
    ...
    showStyledNotification('🎉 CONGRATULATIONS! You have unlocked 7 Days Premium!', 'success');
    saveData();
    renderUI();
  } else {
    alert(`Ad watched! ${appData.adsWatched}/${REWARD_AD_CONFIG.adsRequired} ads completed.`);
    saveData();
  }
}
...
function showStyledNotification(message, type = 'info') {
  ...
  if (type === 'success') {
    notif.style.background = 'linear-gradient(135deg, #11998e 0%, #38ef7d 100%)';
  } else if (type === 'achievement') {
    notif.style.background = 'linear-gradient(135deg, #f093fb 0%, #f5576c 100%)';
  }
...
function showIsekaiGreeting() {
  ...
  showStyledNotification(greeting, 'info');
}
...
function checkAchievements() {
  ...
  if (newAchievements.length > 0) {
    newAchievements.forEach(ach => {
      setTimeout(() => showStyledNotification(`🎉 Achievement Unlocked:
${ach.icon} ${ach.name}!`, 'achievement'), 100);
    });
  }
}

// ==================== RANK UP CHALLENGE SYSTEM ====================
function checkRankUp() {
  const currentLevel = calculateLevel(appData.totalPoints);
  const currentRankIndex = Math.floor(currentLevel / 10);

  // FIX: Check if level is exactly at rank up point AND challenge is not active
  if ((currentLevel % 10) === 9 && currentRankIndex < (RANKS.length - 1) && !appData.hasActiveRankChallenge) {
...
function isChallengeCompleted() {
  ...
  if (challenge.includes('complete 3 high-priority tasks')) {
    const highDone = appData.tasks.filter(t => 
      t.priority === 'high' && t.isDone && new Date(t.doneTimestamp).toDateString() === new Date().toDateString()
    ).length;
    return highDone >= 3;
  }
...
// FIX: Complete Rank Up Challenge - LOCK RANK UNTIL CHALLENGE COMPLETE
function completeRankUpChallenge() {
  ...
  showStyledNotification(`🎉 RANK UP! ⚔️
You've advanced to ${RANKS[nextRankIndex]} Rank!
💎 Bonus: +${pointsNeeded + 100} points!`, 'success');
}

// FIX: Prevent rank up without challenge completion
function canRankUp() {
  return !appData.hasActiveRankChallenge;
}
...
function showRankUpModal() {
  ...
  content.innerHTML = `
    <div style="font-size: 40px; margin-bottom: 15px;">⚡ RANK UP! ⚡</div>
    <div style="font-size: 24px; margin-bottom: 20px; color: #ffd700;">A challenge has appeared to test your resolve</div>
    <div style="background: rgba(255, 215, 0, 0.1); padding: 20px; border-radius: 10px; margin-bottom: 20px; border-left: 4px solid #ffd700;">
      <div style="font-size: 16px; color: #ffd700;">${appData.rankUpChallenge}</div>
    </div>
    <div style="font-size: 14px; margin-bottom: 20px; color: #aaa;">Complete this challenge to advance to <strong style="color: #00ff00;">${RANKS[nextRankIndex]} Rank</strong></div>
    <button id="acceptChallengeBtn" style="
      background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
      color: white;
      border: none;
      padding: 12px 30px;
  ...
  setTimeout(() => {
    if (document.getElementById('rankUpModalOverlay')) {
      modal.remove();
    }
  }, 5000);
}
...
function renderUI() {
  ...
  if (premiumStatus) {
    if (isPremium) {
      const daysLeft = Math.ceil((appData.premiumExpiry - Date.now()) / (1000 * 60 * 60 * 24));
      premiumStatus.textContent = `Premium (${daysLeft}d left)`;
      premiumStatus.style.color = '#00ff00';
    } else {
  ...
  // Update Ads Watched Count & disable button if premium
  const adsCountEl = document.getElementById('adsWatchedCount');
  if (adsCountEl) {
...
function renderTasks() {
  ...
  filtered.forEach((task, idx) => {
    ...
    // FIX: Edit button for tasks
    const editBtn = document.createElement('button');
    editBtn.textContent = '✏️';
    editBtn.className = 'edit-btn';
    editBtn.style.cssText = 'background: #667eea; color: white; border: none; padding: 5px 10px; border-radius: 5px; cursor: pointer; margin: 0 5px;';
    editBtn.onclick = () => editTask(realIdx);

    const deleteBtn = document.createElement('button');
    deleteBtn.textContent = '🗑️';
    deleteBtn.className = 'delete-btn';
    deleteBtn.style.cssText = 'ba
